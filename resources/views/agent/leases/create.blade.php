                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                @if($properties->isEmpty())
                                    <div class="form-text text-warning">
                                        ...
                                        Hiện tại không có bất động sản nào có phòng trống. Vui lòng liên hệ quản lý để thêm phòng mới.
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('lease_payment_cycle') is-invalid @enderror" id="lease_payment_cycle" name="lease_payment_cycle">
                                    ...
                                    <option value="monthly" {{ old('lease_payment_cycle') == 'monthly' ? 'selected' : '' }}>Hàng tháng</option>
                                    <option value="quarterly" {{ old('lease_payment_cycle') == 'quarterly' ? 'selected' : '' }}>Hàng quý</option>
                                    <option value="yearly" {{ old('lease_payment_cycle') == 'yearly' ? 'selected' : '' }}>Hàng năm</option>
                                    <option value="custom" {{ old('lease_payment_cycle') == 'custom' ? 'selected' : '' }}>Tùy chỉnh (nhập số tháng)</option>
                                </select>
                            ...
                            <div class="col-md-6 mb-3" id="lease_custom_months_field" style="display: none;">
                                ...
                                <input type="number" class="form-control @error('lease_custom_months') is-invalid @enderror" 
                                       ...
                                       value="{{ old('lease_custom_months') }}" min="1" max="60" 
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('lease_payment_day') is-invalid @enderror" id="lease_payment_day" name="lease_payment_day">
                                    @for($i = 1; $i <= 31; $i++)
                                        <option value="{{ $i }}" {{ old('lease_payment_day', 1) == $i ? 'selected' : '' }}>
                                            Ngày {{ $i }}
                                        </option>
                                    @endfor
                                </select>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea class="form-control @error('lease_payment_notes') is-invalid @enderror" 
                                          ...
                                          placeholder="Ghi chú về chu kỳ thanh toán...">{{ old('lease_payment_notes') }}</textarea>
...
document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (data.success && data.deposits.length > 0) {
                        ...
                        html += `
                            ...
                            <div class="alert alert-info mb-0">
                                ...
                                <strong>Lưu ý:</strong> Khi tạo hợp đồng mới, hãy xem xét các khoản cọc hiện có để tránh trùng lặp.
                            </div>
                        `;
    ...
    function loadPropertyPaymentCycle(propertyId) {
        ...
        fetch(`/agent/api/leases/properties/${propertyId}/payment-cycle`, {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                const paymentDaySelect = document.getElementById('lease_payment_day');
                const paymentNotesTextarea = document.getElementById('lease_payment_notes');
    ...
    // Load payment cycle for pre-selected property
    document.addEventListener('DOMContentLoaded', function() {
        const propertySelect = document.getElementById('property_id');
        if (propertySelect && propertySelect.value) {
            loadPropertyPaymentCycle(propertySelect.value);
        }
    });
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (typeof Notify !== 'undefined') {
            Notify.info('Đang tạo hợp đồng...', 'Đang xử lý');
        }
    });
