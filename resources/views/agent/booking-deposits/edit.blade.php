...
@section('title', 'Chỉnh sửa đặt cọc')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-edit me-2"></i>Chỉnh sửa đặt cọc #{{ $deposit->reference_number }}
                    </h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('agent.booking-deposits.update', $deposit->id) }}" method="POST" id="depositForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('property_id') is-invalid @enderror" id="property_id" name="property_id" required onchange="loadUnits()">
                                    <option value="">Chọn bất động sản</option>
                                    @foreach($properties as $property)
                                        <option value="{{ $property->id }}" 
                                            {{ (request('property_id') ? request('property_id') == $property->id : $deposit->unit->property_id == $property->id) ? 'selected' : '' }}>
                                            {{ $property->name }} 
                                    ...
                                    @if(isset($units) && $units->count() > 0)
                                        @foreach($units as $unit)
                                            <option value="{{ $unit->id }}" 
                                                {{ $deposit->unit_id == $unit->id ? 'selected' : '' }}>
                                        ...
                                        @if(!$units->contains('id', $deposit->unit_id))
                                            <option value="{{ $deposit->unit_id }}" selected>
                                                {{ $deposit->unit->code }} - {{ $deposit->unit->unit_type }} 
                                                ({{ number_format($deposit->unit->base_rent ?? 0, 0, ',', '.') }} VNĐ/tháng)
                                                - [Đang sử dụng]
                                            </option>
                                        @endif
                                    @else
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('tenant_user_id') is-invalid @enderror" id="tenant_user_id" name="tenant_user_id">
                                    ...
                                    @foreach($tenantUsers as $user)
                                        <option value="{{ $user->id }}" {{ $deposit->tenant_user_id == $user->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('lead_id') is-invalid @enderror" id="lead_id" name="lead_id">
                                    ...
                                    @foreach($leads as $lead)
                                        <option value="{{ $lead->id }}" {{ $deposit->lead_id == $lead->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control @error('amount') is-invalid @enderror" 
                                           id="amount" name="amount" value="{{ old('amount', number_format($deposit->amount, 0, ',', '.')) }}" 
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('deposit_type') is-invalid @enderror" id="deposit_type" name="deposit_type" required>
                                    ...
                                    <option value="booking" {{ old('deposit_type', $deposit->deposit_type) == 'booking' ? 'selected' : '' }}>Đặt cọc</option>
                                    <option value="security" {{ old('deposit_type', $deposit->deposit_type) == 'security' ? 'selected' : '' }}>Cọc an toàn</option>
                                    <option value="advance" {{ old('deposit_type', $deposit->deposit_type) == 'advance' ? 'selected' : '' }}>Trả trước</option>
                                </select>
                                @error('deposit_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local" class="form-control @error('hold_until') is-invalid @enderror" 
                                       id="hold_until" name="hold_until" value="{{ old('hold_until', $deposit->hold_until->format('Y-m-d\TH:i')) }}" required>
                                @error('hold_until')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" 
                                          ...
                                          placeholder="Nhập ghi chú (tùy chọn)">{{ old('notes', $deposit->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <a href="{{ route('agent.booking-deposits.show', $deposit->id) }}" class="btn btn-outline-secondary">
                                <i class="fas fa-times me-1"></i>Hủy
                            </a>
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i>Cập nhật đặt cọc
                            </button>
                        </div>
                    </form>
                </div>
...
function loadUnits() {
    ...
    if (propertyId) {
        // Simple page reload with property_id parameter
