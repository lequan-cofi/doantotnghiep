            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-0 text-gray-800">
                        <i class="fas fa-edit me-2"></i>Chỉnh sửa công tơ đo
                    </h1>
                    <p class="text-muted mb-0">{{ $meter->serial_no }} - {{ $meter->property->name }}</p>
                </div>
                <div class="btn-group">
                    <a href="{{ route('agent.meters.show', $meter->id) }}" class="btn btn-outline-info">
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form id="meterForm" method="POST" action="{{ route('agent.meters.update', $meter->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <a href="{{ route('agent.meters.show', $meter->id) }}" class="btn btn-secondary">
                                <i class="fas fa-times me-1"></i>Hủy
                            </a>
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i>Cập nhật công tơ
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-8">{{ $meter->unit->code }}</div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-sm-4"><strong>Dịch vụ:</strong></div>
                        <div class="col-sm-8">{{ $meter->service->name }}</div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-sm-4"><strong>Ngày tạo:</strong></div>
                        <div class="col-sm-8">{{ $meter->created_at->format('d/m/Y H:i') }}</div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-sm-4"><strong>Cập nhật:</strong></div>
                        <div class="col-sm-8">{{ $meter->updated_at->format('d/m/Y H:i') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <div class="alert alert-warning">
                        ...
                        <ul class="mb-0">
                            <li>Thay đổi loại dịch vụ có thể ảnh hưởng đến tính toán hóa đơn</li>
                            <li>Thay đổi phòng sẽ ảnh hưởng đến hợp đồng thuê</li>
                            <li>Ngày lắp đặt ảnh hưởng đến lịch sử tính tiền</li>
                        </ul>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (propertyId) {
            ...
            .then(data => {
                ...
                if (Array.isArray(data)) {
                    ...
                    // Restore selected unit if it exists in the new list
                    const currentUnitId = '{{ $meter->unit_id }}';
                    if (currentUnitId) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        .catch(error => {
            ...
            Notify.error('Có lỗi xảy ra khi cập nhật công tơ đo');
            submitBtn.innerHTML = originalText;
            submitBtn.disabled = false;
        });
    });
